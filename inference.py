...
if __name__ == "__main__": 
    ...
    args.cfig_path = os.path.join(args.cfig_dir, f"config_infer_{args.model}.yaml")  
    ...
    if cfig['model_from_lightning']:  # when the model is trained with training_lightning.py
        ...
        pl_module = GDPLightningModel.load_from_checkpoint(cfig['save_model_path'], modelname = args.model, cfig = cfig, strict = True)
        model = pl_module.model.to(device)

    else:      # when the model is trained with train.py
        if args.model == 'dosesp':
            model = DoseSP(
    ...
    with torch.no_grad():
        ...
        for batch_idx, data_dict in enumerate(test_loader):
            ...
            outputs, coarse_seg, compact_loss, sep_loss, scribble_loss = model(data_dict['data'].to(device)) 
