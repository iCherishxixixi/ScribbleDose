...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in self.scale_dose_Dict[PatientID].keys():
            if key in ['PTV_High', 'PTV_Mid', 'PTV_Low']:
                opt_dose_dict[self.scale_dose_Dict[PatientID][key]['OPTName']] = \
                    self.scale_dose_Dict[PatientID][key]['PDose'] / self.cfig['dose_div_factor']
                if key != 'PTV_High':
        ...
        # -------------------------
        # Replace ROI masks with scribbles BEFORE combine_*
        # -------------------------
        if self.cfig.get("use_scribble", False):
            scribble_mode = self.cfig.get("scribble_mode", "both")  # "oar" / "ptv" / "both"

            if scribble_mode in ["oar", "both"]:
                for k in need_list:
                    sk = f"{k}_scribble"
                    if sk in In_dict:
                        In_dict[k] = In_dict[sk]

            if scribble_mode in ["ptv", "both"]:
                ptv_keys = list(opt_dose_dict.keys()) + list(dose_dict.keys())
                for k in ptv_keys:
                    sk = f"{k}_scribble"
                    if sk in In_dict:
                        In_dict[k] = In_dict[sk]
        ...
        # -------------------------
        # NOW combine (will use scribbles if replaced)
        # -------------------------
        ...
        # data_dict['supervoxels'] = In_dict['supervoxels']
        ...
        if self.cfig['CatStructures']:
            data_dict['data'] = torch.cat((cat_optptv, cat_ptv, cat_oar, In_dict['Body'], In_dict['img'], data_dict['beam_plate'], data_dict['angle_plate'], prompt_extend, In_dict['background_scribble']), axis=0)
        else:
            data_dict['data'] = torch.cat((comb_optptv, comb_ptv, comb_oar, In_dict['Body'], In_dict['img'], data_dict['beam_plate'], data_dict['angle_plate'], prompt_extend, In_dict['background_scribble']), axis=0) # , In_dict['obj_2DGy'], In_dict['obj_2DWei']    
...
if __name__ == '__main__':
    ...
    cfig = {
        ...
        'val_bs': 1,
        'num_workers': 0,
        'csv_root': '/data/zzx/DosePredictionPolyU/GDP-HMM-Sp/meta_files/meta_data.csv',
        'scale_dose_dict': '/data/zzx/DosePredictionPolyU/GDP-HMM-Sp/meta_files/PTV_DICT.json',
        'pat_obj_dict': '/data/zzx/DosePredictionPolyU/GDP-HMM-Sp/meta_files/Pat_Obj_DICT.json',
        'down_HU': -1000,
        'up_HU': 1000,
        'denom_norm_HU': 500,
        'in_size': (96, 128, 144),
    ...
    for i, batch in enumerate(train_loader):
        ...
        print("[DEBUG] ID:", ID)
        print("[DEBUG] data shape:", x.shape)
        print("[DEBUG] show slice z =", z)
        ...
        for c in range(N):
            axes[c].imshow(x[c, z], cmap="gray")
            axes[c].set_title(f"ch={c}  sum={x[c].sum():.1f}")
            axes[c].axis("off")

        for k in range(N, len(axes)):
            axes[k].axis("off")

        plt.suptitle(f"{ID} | use_scribble={cfig['use_scribble']} | z={z}", fontsize=14)
        plt.tight_layout()
        plt.show()
